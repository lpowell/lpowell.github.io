...
For Each line in dict.Items								'Read every line in dict
	CreateShortcut(line, FolderPath)					'Call CreateShortcut Sub
...
set shortcut = CreateObject("Wscript.Shell").CreateShortcut(CreateObject("Wscript.Shell").SpecialFolders(Genre) & + "\" + FileName + ".lnk")	'Create the shortcut in folder *Genre* with name FileName.lnk
